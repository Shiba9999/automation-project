...
//run command-> node priceCompare.js "iphone11"
//this custom iphone 11 due to use of pName= process.argv[2];
...
(async function fn() {
    try {
        ...
        cTab = allTabsArr[0];
        let list1 = await getListingFromAmazon(links[0], pName);
        console.table(list1);
         list2 = await getListingFromFlipkart(links[1], pName);
        console.table(list2);
         list3 = await getListingFromPaytm(links[2], pName);
        console.table(list3);
    } catch (err) {
        console.log(err);
    }
...
async function getListingFromFlipkart(link, pName) {
    ...
    await cTab.waitForSelector("._30jeq3", { visible: true });
    return cTab.evaluate(fConsoleFn, ".s1Q9rs", "._30jeq3")
    // _4rR01T-> pname
...
async function getListingFromPaytm(link, pName) {
    ...
    return  cTab.evaluate(pConsoleFn, ".UGUy", "._1kMS");

    // .UGUy-> pname
    // ._1kMS-> price
}
function pConsoleFn(pNameSelector, priceSelector) {
    let pNameElems = document.querySelectorAll(pNameSelector);
    let priceElems = document.querySelectorAll(priceSelector);
    let list = [];
    for (let i = 0; i < 5; i++) {
